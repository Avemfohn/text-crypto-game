...
const PlainTextTemplateSingle = (props) => {
    ...
    function wordEntered(letter, correctLetter, cipherLetter) {
        ...
        ////CHECK ALL THE LETTERS ENTERED OR NOT
        var wordInputs = document.getElementsByClassName("wordInput");
        var spaceCount = 0;
        var multipleLetterFilled = 0;
        var differentLetterCount = 0;
        for (let index = 0; index < wordInputs.length; index++) {
            if (wordInputs[index].value === "" || wordInputs[index].value === " ") {
                spaceCount += 1;
            }
            if (wordInputs[index].value.toLocaleUpperCase("tr") !== letter.toLocaleUpperCase("tr") && wordInputs[index].value !== "") {
                console.log(wordInputs[index].value);
                multipleLetterFilled += 1;
            }

        }
        console.error(spaceCount, wordInputs.length)
        if (spaceCount > 0) {
            setGuessAll(true)
        } else {
            setGuessAll(false)
        ...
        for (let index = 0; index < wordInputs.length; index++) {
            if (wordInputs[index].value !== "" && wordInputs[index].value !== " ") {
                currentLettersArray.push(wordInputs[index].value);
            }
        }

        var uniqueArray = [...new Set(currentLettersArray)];
        if (uniqueArray.length === 0 || uniqueArray.length > 1) {
            setWordCheck(true);
        } else {
            setWordCheck(false);
        }
    ...
    function guessAllButtonClicked() {
        ...
        if (wrongCount > 0) {
            for (var index = 0; index < elements.length; index++) {
                elements[index].parentNode.classList.add("wrongGuess");
            }
            setTimeout(() => {
...
export default PlainTextTemplateSingle;
